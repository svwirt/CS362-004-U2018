...
int main() {
    ...
    printf("Running Random Card Test for Council Room\n");
    ...
    for( i=0; i<NUM_TESTS; i++ )
    {
        ...
        cardEffect(council_room, 1, 1, 1, gState, 0, 0);
        ...
        afterBuys = gState->numBuys;
         
         if(afterHand == beforeHand + 4){
         ...
         if(afterBuys == beforeBuys + 1){
            printf("Buy Test %d passed: before-%d after-%d\n", i, beforeBuys, afterBuys);
            passed++;
         }
         else{
            printf("Buy Test %d failed: before-%d after-%d\n", i, beforeBuys, afterBuys);
            failed++;
         }
